...
import torch
import envs
from game import BU_game_dynamics
import collections
from agents import BU_DRQNAgent_2
...
class Hidden(object):
    ...
    def update_game_records(self, p1_records, p2_records, current_player, action, hidden_st, hidden_tar_st):
    ...
    def get_game_records(self, current_player, p1_records, p2_records):
        for key in current_player:
            if key == "Player_One":
                if len(p1_records) == 0:
                    action = 0
...
def run_game(step_count, target_update, train_mode):
    ...
    stacker.player_one_records, stacker.player_two_records = stacker.update_game_records(stacker.player_one_records,
    ...
    while not is_done:
        ...
        mle_stacker.add_mle_data(mpo, spo, tpo, mph, sph, tph)
        bu = compute_bayesian_update(priors1, priors2, priors3)
        p1_msg_records, p2_msg_records = update_msg_records(p1_msg_records, p2_msg_records, current_player, bu)
        ...
        stacker.player_one_records, stacker.player_one_records = stacker.update_game_records(stacker.player_one_records,
                                                                                       stacker.player_two_records,
                                                                                       current_player,
                                                                                       action,
...
def run_one_phase(min_steps, target_update, eval_every_n):
    ...
    while step_count < min_steps:
        ...
        epi_return.append(episode_return)
        ...
        if step_count % eval_every_n == 0:
            ...
            eval_num_episodes += 1
            eval_avg_reward.append(eval_sum_returns / eval_num_episodes)
            eval_avg_steps.append(eval_step_count / eval_num_episodes)
    ...
    epi_return = np.array(epi_return)

    game_results = pd.DataFrame(epi_return, columns=['Results'])
    ...
    training_df.to_csv("BU_Training Scores.csv")
    eval_df.to_csv("BU_Eval Scores.csv")
    loss_df.to_csv("BU_Running Loss.csv")
    game_results.to_csv('BU_Game Results.csv')


    return step_count, sum_returns, num_episodes


run_one_phase(30000, 10, 50)
