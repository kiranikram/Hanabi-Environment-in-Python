...
def apply_action(records, action):
    ...
    hand_p2 = records[1]
    deck = records[2]
    obs_tensor = records[3]
    current_player = records[4]
    own_colour_p1 = records[5]
    own_suit_p1 = records[6]
    own_colour_p2 = records[7]
    own_suit_p2 = records[8]
    fireworks = records[9]
    ...
    updated_records = [hand_p1, hand_p2, deck, obs_tensor, current_player, own_colour_p1, own_colour_p2, own_suit_p1,
...
def get_reward(observations):
    reward = 0
    fireworks_count = []
    fireworks = observations[-1]

    for key in fireworks:
        fireworks_count.append(key)

    for i in fireworks_count:
        reward += int(i[1:2])
...
def is_terminal(observations, reward):
    ...
    elif observations[0] == {} and observations[1] == {} and observations[2] == {}:
        return True
