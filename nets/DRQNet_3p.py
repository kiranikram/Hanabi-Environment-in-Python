...
class CNet(nn.Module):
    def __init__(self):
        ...
        self.oc_1_lookup = nn.Embedding(115, 128)
        self.oc_2_lookup = nn.Embedding(115, 128)
        self.oc_3_lookup = nn.Embedding(115, 128)
        self.os_1_lookup = nn.Embedding(115, 128)
        self.os_2_lookup = nn.Embedding(115, 128)
        self.os_3_lookup = nn.Embedding(115, 128)
        self.th_1_lookup = nn.Embedding(115,128)
        self.th_1_lookup = nn.Embedding(115, 128)
        self.th_1_lookup = nn.Embedding(115, 128)

        self.f_1_lookup = nn.Embedding(115, 128)
        self.f_2_lookup = nn.Embedding(115, 128)
        self.f_3_lookup = nn.Embedding(115, 128)

        # self.state_tensor_lookup = nn.Embedding(48, 128)
        self.i_t_lookup = nn.Embedding(28, 128)
        self.lives_lookup = nn.Embedding(10, 128)

        self.prev_action_lookup = nn.Embedding(90, 128)
    ...
    def forward(self, oc_1, oc_2, oc_3, os_1, os_2, os_3,th_1,th_2,th_3, f_1, f_2, f_3, i_t, lives, hidden, prev_action, agent):
        """
        ...
        z_th_1 = self.oc_1_lookup(th_1)
        z_th_2 = self.oc_2_lookup(th_2)
        z_th_3 = self.oc_3_lookup(th_3)
        ...
        z_o_d = z_oc_1 + z_oc_2 + z_oc_3 + z_os_1 + z_os_2 + z_os_3 + z_th_1 + z_th_2 + z_th_3 + z_f_1 + z_f_2 + z_f_3
        z_o_t = z_i_t + z_lives
