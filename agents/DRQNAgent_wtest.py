...
import collections
import random
from nets import DRQNet, DRQNet_3p
from game import full_game_dynamics_3p, full_game_evolution_3p, full_game_dynamics,full_game_evolution,BU_game_evolution,BU_game_dynamics
from torch.autograd import Variable
from torch.nn.utils import clip_grad_norm_

"""Need to set these for game configuration"""
game = BU_game_dynamics
evolution = BU_game_evolution
...
class Agent(object):
    ...
    def begin_episode(self, current_player, legal_actions, obs_dict, obs_tensor, hidden, hidden_tar):
        ...
        oc_1, oc_2, oc_3, os_1, os_2, os_3,  f_1, f_2, f_3, i_t, lives, act, cp = game.transform_inputs(
            obs_dict,
            obs_tensor,
            prev_action,
            current_player)

        hidden_t, q_s = self.policy_net(oc_1, oc_2, oc_3, os_1, os_2, os_3,  f_1, f_2, f_3, i_t, lives,
                                        hidden, act, cp)
        ...
        hidden_tar_t, q_tar_s = self.target_net(oc_1, oc_2, oc_3, os_1, os_2, os_3,  f_1, f_2, f_3,
                                                i_t, lives,
                                                hidden_tar, act, cp)
    ...
    def step(self, current_player, legal_moves, observation_dict, observation_tensor, prev_action, hidden,
        ...
        if train_mode:
            hidden_tar_t, target_q_s = self.target_net(oc_1, oc_2, oc_3, os_1, os_2, os_3,  f_1, f_2,
                                                       f_3, i_t, lives,
                                                       hidden_tar, act, cp)
