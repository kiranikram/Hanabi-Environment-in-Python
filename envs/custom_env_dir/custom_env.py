...
"""Instructions:
To play with 3P and a 4x4 stack (with and without communication) 
                                                 Set evolution = full_game_evolution_3p
                                                 Set game = fullBU_game_dynamics_3p
                                                 
                                                 Set self.players as the dictionary with three players
                                                 Set self.initial_fireworks as the dictionary with four stacks
                                                 
                            in the step function: Set observations = self.deal_hands_during_game_3p(observations)
                                                  Set observations[6], observations[7], observations[8], 
                                                  observations[9],  observations[10], 
                                                  observations[11] = self.update_perspectives_3p(observations)
                                    
                            in the reset function: Set hand_p1, hand_p2, hand_p3, _, deck = self.deal_initial_hands_3p()
                                                   observations = [hand_p1, hand_p2,hand_p3, deck, opening_obs_tensor, 
                                                   current_player, p1_opc, p1_ops, p2_opc, p2_ops, p3_opc, p3_ops,fireworks]
                                                   
To play with 2P and a 3x3 stack (with and without communication) 
                                                 Set evolution = BU_game_evolution
                                                 Set game = BU_game_dynamics
                                                 
                                                 Set self.players as the dictionary with two players
                                                 Set self.initial_fireworks as the dictionary with three stacks
                                                 
                            in the step function: Set observations = self.deal_hands_during_game(observations)
                                                  Set observations[5], observations[6], observations[7], 
                                                  observations[8] = self.update_perspectives(observations)
                                    
                            in the reset function: Set hand_p1, hand_p2, _, deck = self.deal_initial_hands()
                                                   Set observations = [hand_p1, hand_p2, deck, opening_obs_tensor, 
                                                   current_player, p1_opc, p1_ops, p2_opc, p2_ops, fireworks] """
...
class HanabiEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.players = {"Player_One": 98, "Player_Two": 99}  # for two players
        self.unuse_players = {"Player_One": 98, "Player_Two": 99, "Player_Three": 97}  # for three players
    ...
    def reset(self):
        ...
        # hand_p1, hand_p2, hand_p3, _, deck = self.deal_initial_hands_3p()
        ...
        # observations = [hand_p1, hand_p2,hand_p3, deck, opening_obs_tensor, current_player,
        # p1_opc, p1_ops, p2_opc, p2_ops, p3_opc, p3_ops,fireworks]
