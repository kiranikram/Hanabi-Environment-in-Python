...
class HanabiEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = BU_game_evolution.get_reward(observations)

        done = BU_game_evolution.is_terminal(observations, reward)
    ...
    def apply_action_to_state(self, action):
        ...
        updated_observations = BU_game_evolution.apply_action(records, action)
    ...
    def update_perspectives(observations):
        ...
        if len(observations[5]) < 3:
            player = 1
            observations[5] = BU_game_evolution.check_and_deal_col(observations[5], player)
        if len(observations[6]) < 3:
            player = 1
            observations[6] = BU_game_evolution.check_and_deal_suit(observations[6], player)
        if len(observations[7]) < 3:
            player = 2
            observations[7] = BU_game_evolution.check_and_deal_col(observations[7], player)
        if len(observations[8]) < 3:
            player = 2
            observations[8] = BU_game_evolution.check_and_deal_suit(observations[8], player)
